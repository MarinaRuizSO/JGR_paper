...
def run_full_ksn_delta_case_iii(map_ax, map_fig, label_count, case_labels,cbar_labels, subplot_letter, mountain_count, DEM_paths, file_names,mountain_range_names, base_path):
    ...
    for file in filelist[:]:
        ...
        for f in glob.glob(DEM_paths[mountain_count]+f'{basin_name}_original_rain__chi_map_theta_best_*.csv'):
        ...
        for f in glob.glob(DEM_paths[mountain_count]+f'{basin_name}_no_rainfall__chi_map_theta_best_*.csv'):
            df_basins_without_rain_theta_best = pd.read_csv(f)

        df_no_nan_without_rain_theta_best = df_basins_without_rain_theta_best[df_basins_without_rain_theta_best.m_chi >= 0]
        ...
        # processing follows the same method as for ksn distortion calculations
        # 1 - normalise the drainage area and remove values that are below 0.1 
        df_no_nan_without_rain_theta_best.drainage_area/=np.max(df_no_nan_without_rain_theta_best.drainage_area)
        df_basins_without_rain_theta_best_da_0_1 = df_no_nan_without_rain_theta_best[df_no_nan_without_rain_theta_best.drainage_area>=0.1]
        # 2 - normalise the m_chi (ksn) values by the highest in the basin
        #breakpoint()
        df_basins_without_rain_theta_best_da_0_1.m_chi/=np.max(df_basins_without_rain_theta_best_da_0_1.m_chi)
        df_basins_without_rain_theta_best_da_0_1['m_chi_norm'] = df_basins_without_rain_theta_best_da_0_1.m_chi/df_basins_without_rain_theta_best_da_0_1.m_chi.max()

        df_basins_delta_ksn = np.abs(df_basins_with_rain_theta_best_da_0_1.m_chi_norm - df_basins_without_rain_theta_best_da_0_1.m_chi_norm)
        df_basins_delta_ksn_no_nan = df_basins_delta_ksn[df_basins_delta_ksn>=0]

        # inner join with one of the datasets 
        delta_ksn_joined = pd.merge(df_basins_with_rain_theta_best_da_0_1, df_basins_delta_ksn_no_nan, left_index=True, right_index=True)
    ...
    cbar.set_label(label=cbar_labels[label_count])#r'$|k^{*}_{sn,\theta=\theta_{best}}- k^{*}_{sn-q,\theta=\theta_{best}}|$', weight='normal')
    ...
    #plt.show()
    #plt.savefig(file_path+f'{mountain_range_names[i]}_original_rain_all_basins_delta_case_iii_ksn_normalisedTEST.jpg', dpi=500,  bbox_inches='tight')
