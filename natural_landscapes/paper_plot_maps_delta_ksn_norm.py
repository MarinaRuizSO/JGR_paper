...
def run_full_ksn_delta_case_iq(map_ax, map_fig, label_count, case_labels,cbar_labels, subplot_letter, mountain_count, DEM_paths, file_names,mountain_range_names, base_path):
    ...
    cbar.set_label(label=cbar_labels[label_count])# |k^{*}_{sn-q,\theta=0.45}- k^{*}_{sn-q,\theta=\theta_{best}}|$')
    ...
    # axins = inset_axes(map_ax, width="30%", height="30%", loc=f'{latlon_df[mountain_range_names[i]][4]}', 
    ...
    #plt.show()
    #plt.savefig(file_path+f'{mountain_range_names[i]}_original_rain_all_basins_delta_ksn_normalised.jpg', dpi=500,  bbox_inches='tight')
