...
def run_full_ksn_delta_case_ii(map_ax, map_fig, label_count, case_labels,cbar_labels, subplot_letter, mountain_count, DEM_paths, file_names,mountain_range_names, base_path):
    ...
    for file in filelist[:]:
        ...
        df_basins_with_rain_theta_45 = pd.read_csv(DEM_paths[mountain_count]+f'{basin_name}_original_rain__chi_map_theta_0_45.csv')
        df_no_nan_theta_45 = df_basins_with_rain_theta_45[df_basins_with_rain_theta_45.m_chi >= 0]
    
        df_basins_without_rain_theta_45 = pd.read_csv(DEM_paths[mountain_count]+f'{basin_name}_no_rainfall__chi_map_theta_0_45.csv')
        df_no_nan_without_rain_theta_45 = df_basins_without_rain_theta_45[df_basins_without_rain_theta_45.m_chi >= 0]
        ...
        # processing follows the same method as for ksn distortion calculations
        # 1 - normalise the drainage area and remove values that are below 0.1 
        df_no_nan_without_rain_theta_45.drainage_area/=np.max(df_no_nan_without_rain_theta_45.drainage_area)
        df_basins_without_rain_theta_45_da_0_1 = df_no_nan_without_rain_theta_45[df_no_nan_without_rain_theta_45.drainage_area>=0.1]
        # 2 - normalise the m_chi (ksn) values by the highest in the basin
        #breakpoint()
        df_basins_without_rain_theta_45_da_0_1.m_chi/=np.max(df_basins_without_rain_theta_45_da_0_1.m_chi)
        df_basins_without_rain_theta_45_da_0_1['m_chi_norm'] = df_basins_without_rain_theta_45_da_0_1.m_chi/df_basins_without_rain_theta_45_da_0_1.m_chi.max()

        df_basins_delta_ksn = np.abs(df_basins_with_rain_theta_45_da_0_1.m_chi_norm - df_basins_without_rain_theta_45_da_0_1.m_chi_norm)
        df_basins_delta_ksn_no_nan = df_basins_delta_ksn[df_basins_delta_ksn>=0]

        # inner join with one of the datasets 
        delta_ksn_joined = pd.merge(df_basins_with_rain_theta_45_da_0_1, df_basins_delta_ksn_no_nan, left_index=True, right_index=True)
    ...
    cbar.set_label(label=cbar_labels[label_count])#r'$|k^{*}_{sn,\theta=0.45}- k^{*}_{sn-q,\theta=0.45}|$', weight='normal')
    ...
    # lonW = int(latlon_df[mountain_range_names[i]][0])
    # lonE = int(latlon_df[mountain_range_names[i]][1])
    # latS = int(latlon_df[mountain_range_names[i]][2])
    # latN = int(latlon_df[mountain_range_names[i]][3])
    # #-13.6927, -69.9541
    ...
    #plt.show()
    #plt.savefig(file_path+f'{mountain_range_names[i]}_original_rain_all_basins_delta_case_ii_ksn_normalisedTEST.jpg', dpi=500,  bbox_inches='tight')
    #mountain_count+=1